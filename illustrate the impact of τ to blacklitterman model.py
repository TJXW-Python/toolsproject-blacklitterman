...
def optimization_adding_views(Vp,view,view_link,Pi,rf):
    ...
    def pi_add_view(Vp,view,view_link,Pi):
        # scaling factor for blacklitterman model is set bewtween a range
        scalar = numpy.linspace(0.025, 0.05, 6)      
        ...
        len_list = len(scalar_list)
        # Obtain a list of pi_new values
        pi_new_list = []
        for i in range(len_list):
            pi_vol = dot(scalar_list[i],Vp)                    
            view_vol= dot(dot(dot(scalar_list[i],view_link),Vp),transpose(view_link))  
